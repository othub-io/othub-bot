...
module.exports = spamCheck = async (command) => {
  if(command){
    //check for spam
    const spam_result = await db.prepare('SELECT date_last_used FROM command_history WHERE command = ?').get(command);
    ...
    if(timeDif > cooldown){
      ...
      await db.prepare('REPLACE INTO command_history VALUES (?,?)').run(command, time_stamp);
